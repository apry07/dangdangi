...
async def on_ready():
    ...
    print("(dangdangi ver:1.9.1): ready")
    print("apry_dangdangi_xms128m_xmx16G")
    game = discord.Game("dangdangi 1.9.1v")
...
async def on_message(message):
    ...
    if message.content.startswith("댕댕아 ㅎ"):
    ...
    if message.content.startswith("/디버그"):
        await message.channel.send("terminal.debug      877362254654353478 (dangdangi ver:1.9.1): ready apry_dangdangi_xms128m_xmx16G")
    
    if message.content.startswith("/update dangbt"):
        await message.channel.send("1.9.1v 최신버전입니다. : vsc를 참고해주세요")
...
client.run("ODc3MzYyMjU0NjU0MzUzNDc4.YRxhRg.HnkZw-jsy_-pQB9o30XDu8MpkqY")